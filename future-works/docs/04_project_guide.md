...
sequenceDiagram
    ...
    Note over ESP: 1. อ่านค่า Sensor (ทุก 5 วินาที)
    ESP->>MQTT: Publish: sensors/temperature/esp32-003

    par ส่งข้อมูลไป 2 ทาง
        MQTT->>NR: อัปเดตหน้าปัด (Gauge)
        MQTT->>Py: ส่งให้ Python ประมวลผล
    end

    Note over Py: 2. ตรวจสอบเงื่อนไข
    alt ร้อนเกิน (> 30°C)
        Py->>MQTT: Publish Command: {"value": 1}
        MQTT->>ESP: สั่งเปิด พัดลม (ON)
        Py->>API: บันทึก Log "switch_on"
    else เย็นลง (< 25°C)
        Py->>MQTT: Publish Command: {"value": 0}
        MQTT->>ESP: สั่งปิด พัดลม (OFF)
        Py->>API: บันทึก Log "switch_off"
    end
```
